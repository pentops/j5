...
import (
	...
	"github.com/pentops/j5/internal/gen/test/query/v1/query_testpb"
...
func TestDefaultFiltering(t *testing.T) {
	...
	ss.Step("List Page", func(ctx context.Context, t flowtest.Asserter) {
		...
		for _, state := range res.Foo {
			if state.Status != query_testpb.FooStatus_ACTIVE {
				t.Fatalf("expected status %s, got %s", query_testpb.FooStatus_ACTIVE, state.Status)
			}
		}
...
func TestFilteringWithAuthScope(t *testing.T) {
	...
	ss.Setup(func(ctx context.Context, t flowtest.Asserter) error {
		...
		uu.SetupFoo(t, 20, func(idx int, foo *TestObject) {
			...
			createdAt := time.Now()
			foo.SetScalar(pquery.JSONPath("createdAt"), createdAt.Format(time.RFC3339Nano))
	...
	ss.Step("List Page", func(ctx context.Context, t flowtest.Asserter) {
		...
		for ii, state := range res.Foo {
			t.Logf("%d: %s (%s)", ii, state.Data.Field, state.CreatedAt.AsTime().Format(time.RFC3339Nano))
		}
...
func TestDynamicFiltering(t *testing.T) {
	...
	t.Run("Oneof filter - happy", func(t *testing.T) {
		...
		for ii, event := range res.Foo {
			...
			case *query_testpb.FooState_Data_Shape_Circle_:
			default:
