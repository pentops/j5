...
import (
	...
	"github.com/pentops/j5/lib/j5query/internal/dbconvert"
...
func (uu *SchemaUniverse) FooLister(t flowtest.TB, mods ...func(*pquery.TableSpec)) *pquery.Lister {
	...
	requestSchema, ok := (&query_testspb.FooListRequest{}).J5Object().RootSchema()
	if !ok {
		t.Fatal("failed to get request schema")
	}
	responseSchema, ok := (&query_testspb.FooListResponse{}).J5Object().RootSchema()
	if !ok {
		t.Fatal("failed to get response schema")
	}
	fooSchema, ok := (&query_testpb.FooState{}).J5Object().RootSchema()
	if !ok {
