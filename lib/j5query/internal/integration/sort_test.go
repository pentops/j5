...
func TestSortingWithAuthScope(t *testing.T) {
	...
	ss.Setup(func(ctx context.Context, t flowtest.Asserter) error {
		queryer = uu.FooLister(t, func(spec *j5query.TableSpec) {
			spec.Auth = tokenAuth()
		})
		uu.SetupFoo(t, 20, func(idx int, foo *TestObject) {
			...
			foo.SetScalar(j5query.JSONPath("data", "characteristics", "height"), height)
			foo.SetScalar(j5query.JSONPath("data", "characteristics", "length"), length)
			createdAt := time.Now()
			foo.SetScalar(j5query.JSONPath("createdAt"), createdAt.Format(time.RFC3339Nano))

			label := fmt.Sprintf("foo %d (weighted %d, height %d, length %d)", ii, weight, height, length)
			foo.SetScalar(j5query.JSONPath("data", "field"), label)
...
func TestDynamicSorting(t *testing.T) {
	...
	uu.SetupFoo(t, 30, func(ii int, foo *TestObject) {
		...
		if err := SetScalar(profile1, j5query.JSONPath("name"), fmt.Sprintf("profile %d", ii)); err != nil {
			t.Fatalf("failed to set profile name: %v", err)
		}

		if err := SetScalar(profile1, j5query.JSONPath("place"), int64(ii)+50); err != nil {
			t.Fatalf("failed to set profile place: %v", err)
		}

		if err := SetScalar(profile2, j5query.JSONPath("name"), fmt.Sprintf("profile %d", ii)); err != nil {
			t.Fatalf("failed to set profile name: %v", err)
		}

		if err := SetScalar(profile2, j5query.JSONPath("place"), int64(ii)+15); err != nil {
			t.Fatalf("failed to set profile place: %v", err)
		}

		switch ii {
		case 10:
			foo.SetScalar(j5query.JSONPath("data", "shape", "circle", "radius"), int64(10))
		case 11:
			foo.SetScalar(j5query.JSONPath("data", "shape", "square", "side"), int64(10))
		}
	})
