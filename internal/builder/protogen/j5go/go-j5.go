...
func generateFile(sc *j5schema.SchemaCache, gen *protogen.Plugin, file *protogen.File) error {
	err := generateBaseFile(sc, gen, file)
	if err != nil {
		return err
	}

	stateSets, err := state.WalkFile(sc, file)
	if err != nil {
		return err
	}
	...
	querySets, err := query.WalkFile(sc, file)
	if err != nil {
		return err
	}
...
func generateBaseFile(sc *j5schema.SchemaCache, gen *protogen.Plugin, file *protogen.File) error {
...
func genMessage(sc *j5schema.SchemaCache, g *protogen.GeneratedFile, message *protogen.Message) error {
	schema, err := sc.Schema(message.Desc)
	if err != nil {
		return err
	}
