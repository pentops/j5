...
func (ss *Service) linkSchemas(schemaSet *j5reflect.PackageSet) error {
	for _, method := range ss.Methods {
		...
		if err := method.Response.link(schemaSet); err != nil {
			return patherr.Wrap(err, "response", method.Response.FullName())
		}
	}
	return nil
}
...
func (mm *Method) ToJ5Proto() (*schema_j5pb.Method, error) {
	...
	if requestSchema == nil {
		return nil, fmt.Errorf("request schema %q not linked", mm.Request.FullName())
	}
	...
	responseSchema := mm.Response.Schema()
	if requestSchema == nil {
		return nil, fmt.Errorf("response schema %q not linked", mm.Request.FullName())
	}
