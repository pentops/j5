...
func (ww *Walker) walkFile() (*File, error) {
	...
	for {
		...
		default:
			...
			if err != nil {
				if ue, ok := err.(*unexpectedTokenError); ok {
					return file, &unexpectedTokenError{
						context:  "root",
						tok:      ue.tok,
						expected: append(ue.expected, lexer.PACKAGE, lexer.IMPORT),
					}
				}
			...
			if err != nil {
				if ue, ok := err.(*unexpectedTokenError); ok {
					return &unexpectedTokenError{
						context:  "body",
						tok:      ue.tok,
						expected: append(ue.expected, lexer.RBRACE, lexer.EXPORT, lexer.INCLUDE),
					}
				}
				return err
			}
...
func (ww *Walker) nextStatement(body *Body) error {
	...
	case lexer.IDENT:
		...
		ref, err := ww.popReference()
		if err != nil {
			return patherr.Wrap(err, ref.String())
		}
...
func (ww *Walker) walkStatement(ref Reference) (Statement, error) {
	...
	if ww.nextType() == lexer.COLON {
		ww.popToken()
		// <reference>:...
		// Expect a qualifier reference, e.g an object:a.b.C
		block.Qualifier, err = ww.popReference()
		if err != nil {
			return nil, err
		}
	}
	...
	case lexer.DESCRIPTION:
		...
		block.Description = ww.popToken().Lit
		block.End = ww.currentPos()
		return block, nil
